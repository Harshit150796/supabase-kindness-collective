...
export function CouponFruit({ branchTip, data, state, groundY, index, onLanded, onRegrown, onClickHanging, isMobile = false, labelSuppressed = false }: Props) {
  ...
  const texture = useMemo(
    () => drawCouponTexture(data, brandLogos[data.brand]?.logo),
    [data]
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* The coupon face is baked into the mesh texture (brand stripe, logo badge,
            trait pill, amount) on every device. The previous CSS3D <Html transform>
            overlay could not be depth-sorted against the tree, so brand faces showed
            through leaves and flickered while the tree swayed. */}
