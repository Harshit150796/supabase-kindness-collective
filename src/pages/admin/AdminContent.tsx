...
export default function AdminContent() {
  ...
  const handleSave = async (id: string) => {
    ...
    const { error } = await supabase
      .from('cms_content')
      .update({ content_value: editValues[id] })
      .eq('id', id);
    setSaving(false);
    if (error) {
  ...
  const grouped = SECTIONS.map(section => ({
    section,
    items: (allContent || []).filter((c: any) => c.section === section),
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 text-muted-foreground">No content yet. Add your first content item above.</div>
        ) : (
                  ...
                  <div key={item.id} className="space-y-2 p-4 rounded-lg bg-muted/50">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => handleDelete(item.id)}><Trash2 className="w-3 h-3" /></Button>
